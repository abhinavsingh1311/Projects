...
export default function Home() {
  ...
  useEffect(() => {
    async function fetchUserAndResumes() {
      try {
        ...
        if (currentUser) {
          // Fetch resumes with joined analysis data
          ...
          // Fetch job matches if there are any resumes
          if (processedResumes.length > 0) {
            fetchJobMatches(processedResumes[0].id);
          }
        }
      }
      catch (error) {
  ...
  // Calculate dashboard statistics from resume data
  const calculateDashboardStats = (resumeData) => {
    ...
    setDashboardStats({
      ...
      topSkills,
      matchedJobs: 0, // Will be updated when job matches are fetched
      averageScore: avgScore
    });
  };

  // Fetch job matches for a resume
  const fetchJobMatches = async (resumeId) => {
    try {
      ...
      // Update dashboard stats with job match count
